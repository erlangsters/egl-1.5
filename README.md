...
> version 1.4 and before ever existed and the API of this binding is set in
> stone.
...
Also, you will need to use ANGLE on macOS and Windows.
...
OpenGL binding to work with.

Update `rebar.config` to add it to the dependencies along with with the EGL
binding itself.
...
If you happen to use [Erlang.mk](https://erlang.mk), update the `Makefile` instead.
...
In order for your project to compile fine, you will first need to install the
EGL and OpenGL libraries on your system. It's heavily platform-dependent and
therefore instructions cannot be provided here. Also, if you're using macOS or
Windows, you will want to use [ANGLE](https://github.com/google/angle).
...
From there, you will need two things. First, understanding the API mapping (to
know how a piece of C code using EGL translates to Erlang or Elixir), then
understanding thread-safety (how the concept of OpenGL contexts apply at the
BEAM level).

They're explained in the next two sections.

## API mapping

Since it's a direct binding to the C API, it's assumed you have knowledge of
the original API.

Most of the time, all functions translate nicely, equiped with documentation
and common sense.
However, it can be interested to look at it in a more formal way.

To be written.

Finally, the test suite in this repository is also a great reference when you
need a working sample of an EGL function.
