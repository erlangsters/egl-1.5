...
ERL_NIF_TERM egl_execute_command(
    ...
    printf("egl_execute_command:begin\n");
    ...
    if (my_execute_command_if_executor(&pid, function, env, argc, argv, &result)) {
        printf("command has executed on executor thread\n");
        ...
        printf("command has executed on caller OS thread\n");
    }
...
{
    printf("nif_make_current\n");
    ErlNifPid pid;
    enif_self(env, &pid);
    printf("nif_make_current\n");
    bool hasCreatedNewExecutor = my_create_executor_if_not_any(&pid);
    printf("hasCreatedNewExecutor: %d\n", hasCreatedNewExecutor);
